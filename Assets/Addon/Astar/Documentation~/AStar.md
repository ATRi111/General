...
- 给定一个节点**A**及作为父节点的邻居**7**，考虑任意一个其他邻居**X**：
  - 如果不论障碍物情况如何，**7→X**的最短路径都经过**A**，那么这种情况下称**X**为**A**的**自然邻居**
  - 如果是因为特定位置存在障碍物，**7→X**的最短路径才经过**A**，那么这种情况下称**X**为**A**的**强制邻居**
...
- **JPS算法非常不擅长处理地图非常开阔的情况，会在提前找强制邻居的过程中浪费大量时间**
  - **“懒扫描”：检查对角线上的点时，不立刻沿非对角线上扫描，而是把对角线上的点和非对角线上的相邻点直接添加到后续节点**
