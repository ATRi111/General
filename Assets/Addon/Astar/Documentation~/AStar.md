...
*左：0为自然邻居，1为强制邻居；右：0，1，2为自然邻居，3为强制邻居*
...
- JPS仅改变A*中获取可达节点的逻辑，给定**A**和**7**：
  - 如果**A**为起点，在八个方向上**寻找路径点**
  - 如果**7→A**是对角线，在0，1，2方向上**寻找路径点**
  - 如果**7→A**是水平线，在0方向上**寻找路径点**
- **寻找路径点**：沿0方向不断前进，直到遇到满足条件的点，将该节点以及检测过程中找到的关键节点一并加入路径
  - 终点或有**至少一个强制邻居**的节点满足条件
  - 特别地，如果**7→A**是对角线，从当前点出发递归地在水平和竖直方向上寻找路径点，如果找到了，那么当前点也满足条件

- 不满足条件的节点不持久化存储，以降低内存开销
